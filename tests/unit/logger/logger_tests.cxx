...
/* Tests required for the logger class.
  1. Check for new file creation.
  2. Test simple log.
  3. Test multiple logs.
  4. Test multiple logs with different log levels.
  5,
...
protected:
  ...
  bool logFileExists(const std::string& logFile)
  {
    std::ifstream f(logFile.c_str());
    return f.good();
  }

  bool deleteLogFile(const std::string& logFile)
  {
  ...
  std::ifstream& goToLine(std::ifstream& file, unsigned line)
  {
  ...
  std::size_t getMsgPosition(std::string& line)
  {
